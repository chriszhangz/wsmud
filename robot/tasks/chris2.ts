...
const qnjs = /^(qnjs|q)\s(\d+)\s(\d+)\s(\D+)$/i;
const sxjs = /^(sxjs|s)\s(\d+)\s(\D+)$/i;
const lxjs = /^(lxjs|l)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\D+)$/i;
const dzjs = /^(dzjs|d)\s(\d+)\s(\d+)\s(\d+)$/i;
const look = /^(look|l)\s(\D+)$/i;
const check = /^(check|c)\s(\D+)$/i;
const message = /^(message|m)\s([\S\s]*)$/i;
const help = /^(help|h|帮助|指令)$/i;
const helpDetail = /^(help|h)\s(\D+)$/i;
const top = /^(top|t)$/i;
const xy = /^(xy|x)$/i;
const boss = /^(boss|b)$/i;
    ...
    async start(session: Session, config: UserConfig) {
        ...
        async function processMsg(data: Msg) {
            if (data.ch === "rumor") {
                ...
                } else if ((matches = fuli.exec(data.content)) != null) {
                ...
                if (new Date().getTime() - lastchat.getTime() > 1000 * 5 && data.name != "" && data.name != "江湖精灵") {
                    ...
                    if (content == '白如盈' || (content.indexOf('白如盈') >= 0 && content.indexOf('你好') >= 0)) {
                    ...
                    } else if ((matches = boss.exec(data.content)) != null) {
                        if (newbook) {
                            await session.sendAsync(`${GetChinaTime()}` + positions);
                        } else {
                        ...
                        lastchat = new Date();
                    } else if ((matches = xy.exec(data.content)) != null) {
                        if (newXYs) {
                            await session.sendAsync(`jh fam 8`);
                            // var time = new Date().getTime() - lastXYs.getTime();
                            // time = time / 1000;
                            ...
                            await session.sendAsync(`${ch} 😉抱歉，我刚升级完毕,将等待下一次襄阳开启后开始计时。`);
                        }
                        lastchat = new Date();
