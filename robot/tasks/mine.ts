...
export class MineTask extends Task {
    ...
    private cmds: cmd[] = [{content:"jh fam 1 start",type:1},{content:"go north",type:1},{content:"go south",type:0}
    ...
    {content:"go northeast",type:1},{content:"go southeast",type:0},{content:"go northwest",type:0},{content:"go north",type:1},
        ...
        async function processBoss( cmdss:cmd[] ) {
            for (let i = 0; i < cmdss.length; i++) {
                ...
                for(const item in items){
                    ...
                    searchBoss(items[item].name,roomData.name);
                    }
                }
            }
            ...
            if (data.ch === "rumor") {
               ...
               if (new Date().getTime() - lastchat.getTime() > 1000 * 8 && data.name!="" && data.name!="江湖精灵") {
                   ...
                   if (content === "boss" ||content === "b" ){
                       if(newbook){
                           let positons = '';
                           if(new Date().getTime() - lastbook.getTime() > 1000 * 8*60*10){
                            for (const key in dic) {
                                if (dic[key]!="") {
...
function searchBoss(name:string,position:string){
    for (const key in dic) {
        if (name.indexOf(key)>=0&&dic[key]=="") {
            dic[key]=position;
            //console.log('found boss '+name+' at '+dic[key]);
            break;
        }
    }
}
